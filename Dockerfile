...
RUN apk add --no-cache ca-certificates openssl unzip unrar p7zip-full libffi-dev libssl-dev py3-cryptography py3-six py3-cffi py3-chardet py3-requests \
					   libgomp \
...
RUN apk add --no-cache --virtual temp build-base automake autoconf python3-dev py3-pip alpine-sdk \
    ...
    && pip --no-cache-dir install --upgrade cheetah3 sabyenc3 requests pynzb apprise enum34 feedparser configobj cheroot==8.4.3 cherrypy portend notify2 openssl \
